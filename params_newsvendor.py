...
def get_params(n_items, seed_number):
    ...
    for i in range(0, n_items):
        ...
        q = 40 + np.random.randint(-30, 30) # Fixed cost for each item
        qs = 180 + np.random.randint(-80, 80)  # Shortage cost fpr each item
        qw = 120 + np.random.randint(-50, 50) # Excess cost for each item
    ...
    params['q'] = df_parameters['c'].tolist()
